...
class DiscountCell: UICollectionViewCell {
    ...
    private let subTitleLabel = UILabel().then {
        $0.text = "2천원이상 또는 15%이상 할인중"
        $0.font = .size17
        $0.textColor = .baeminGray
    }

    private let subTitleImageView = UIImageView().then {
        $0.image = .image
    ...
    private let dicountImageView = UIImageView().then {
        $0.image = .image
    }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        cv.register(DiscountItemCell.self, forCellWithReuseIdentifier: DiscountItemCell.identifier)
        return cv
    }()
    ...
    private func setupUI() {
        ...
        contentView.addSubviews(titleLabel, subTitleStackView, infoButton, viewAllButton, collectionView, bottomDividerView)
        ...
        subTitleStackView.addArrangedSubviews(subTitleLabel, subTitleImageView)
...
extension DiscountCell: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: DiscountItemCell.identifier,
            for: indexPath
        ) as! DiscountItemCell
...
class DiscountItemCell: UICollectionViewCell {
   ...
   private let deliveryFeeLabel = UILabel().then {
    ...
    func configure(with store: Store) {
        ...
        let badgeWidth = (180 - 8) / 3  // 셀 너비(180)의 1/3 - 여백
